...
bool ImGui::Tabs(const char* label, const ImVec2& size_arg, bool selected, ImColor animCRegular, ImColor animCSelected, ImGuiButtonFlags flags)
{
    ...
    auto it_anim = tab_anim.find(id);

    if (it_anim == tab_anim.end())
    {
        tab_anim.insert({ id, tab_state() });
    ...
    // Render
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, GetColorU32(it_anim->second.bg_col), true, 0.f);
...
static std::map<ImGuiID, key_struct> key_anim;
...
bool ImGui::Keybind(const char* label, int* key)
{
    ...
    const ImGuiID id = window->GetID(label);
    float width = (GetContentRegionMax().x - style.WindowPadding.x) - 15;

    ImVec2 mode_size;
    ImVec2 key_size;

    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(width, 30));
    ImRect mode_rect(window->DC.CursorPos + ImVec2(width - 0, 0), window->DC.CursorPos + ImVec2(width, 40));
    ImRect key_rect;
    ...
    static int controllerButton;

    if (*key != 0 && *key <= 200 && g.ActiveId != id) {
        active_key += Keys[*key];
    ...
    ImVec2 label_size = CalcTextSize(buf_display, NULL, true);

    bool hovered = ItemHoverable(rect, id);

    auto it_anim = key_anim.find(id);

    if (it_anim == key_anim.end())
    {
        key_anim.insert({ id, key_struct() });
        it_anim = key_anim.find(id);
    }

    it_anim->second.text = ImLerp(it_anim->second.text, g.ActiveId == id ? ImColor(200, 200, 200, 255) : ImColor(113, 123, 134), g.IO.DeltaTime * 6.f);

    window->DrawList->AddText(rect.Min + ImVec2(10, 5), GetColorU32(ImGuiCol_Text), label);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(key_rect.Min - ImVec2(60, 31), key_rect.Max + ImVec2(CalcItemWidth() + 38, 16), buf_display, NULL, NULL, ImVec2(0.5f, 0.5f));
    PopStyleColor();
...
bool ImGui::Checkbox(const char* label, bool* v, ImColor bgC, ImColor onC, ImColor offC)
{
    ...
    const float square_sz = 10, checkmark = 10.f;
    const float w = (GetContentRegionMax().x - style.WindowPadding.x) - square_sz - 15;
    const ImVec2 pos = window->DC.CursorPos;
    ...
    auto it_anim = check_anim.find(id);

    if (it_anim == check_anim.end())
    {
        check_anim.insert({ id, checkbox_struct() });
        it_anim = check_anim.find(id);
    }
    ...
    ImGui::GetWindowDrawList()->AddText(ImVec2(check_bb.Max.x - square_sz, check_bb.Min.y), GetColorU32(ImGuiCol_Text), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(150, 0));
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = ((GetContentRegionMax().x - style.WindowPadding.x - 170));
    const float y_size = 32;

    const ImRect bb(window->DC.CursorPos + ImVec2(w / 1.8, 0), window->DC.CursorPos + ImVec2(w, 20));
    const ImRect rect(window->DC.CursorPos + ImVec2(w / 1.8, -5), window->DC.CursorPos + ImVec2(w, (y_size - 5)));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.opened_combo ? ImColor(200, 200, 200, 255) : ImColor(113, 123, 134), g.IO.DeltaTime * 6.f);
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 32) + 5 : 0.f, g.IO.DeltaTime * 12.f);

    GetWindowDrawList()->AddText(ImVec2(window->DC.CursorPos.x + 10, rect.Min.y + (22 - CalcTextSize(label).y) / 2 + 4), GetColorU32(ImGuiCol_Text), label);

    PushClipRect(rect.Min, rect.Max, true);
    GetWindowDrawList()->AddText(ImVec2(rect.Min.x + ((y_size - CalcTextSize(label).y) / 2 + 5), rect.Min.y + (y_size - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), preview_value);
    PopClipRect();

    GetWindowDrawList()->AddRectFilled(rect.Max - ImVec2(y_size, y_size), rect.Max, GetColorU32(it_anim->second.background), 2.f);
    ...
    PushStyleColor(ImGuiCol_WindowBg, ImVec4(ImColor(13, 23, 37, 100)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(10, -5));

    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = GetContentRegionMax().x - style.WindowPadding.x - 20;
    const float y_size = 20;
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ...
    const ImRect slider_bb(window->DC.CursorPos + ImVec2((w / 1.8f), (y_size / 2) - 1), window->DC.CursorPos + ImVec2(w, (y_size / 2) + 7));
    ...
    bool hovered = ItemHoverable(slider_active, id), held, pressed = ButtonBehavior(slider_active, id, &hovered, &held, NULL);
    bool temp_input_is_active = temp_input_allowed && TempInputIsActive(id);
    ...
    GetWindowDrawList()->AddRectFilled(slider_bb.Min + ImVec2(0, 0), slider_bb.Max + ImVec2(0, 1) - ImVec2(5,1), ImColor(13, 23, 37, 255), 30.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddRectFilled(slider_bb.Min + ImVec2(0, 1), ImVec2(it_anim->second.slow + frame_bb.Min.x + 3 - it_anim->second.circle_pump, grab_bb.Max.y + 1), ImColor(57, 186, 186, 255), 30.f);
    }

    PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(175, 184, 199)));
    RenderTextClipped(slider_bb.Min + ImVec2(0, -3), slider_bb.Max + ImVec2(0, 1) - ImVec2(5, 1), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.1f));
    PopStyleColor();

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w, frame_bb.Max.y - ((y_size + CalcTextSize(label).y) / 2 - 3)), GetColorU32(ImGuiCol_Text), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(210, 0));
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() - 65, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(13, 23, 37, 255), true, style.FrameRounding);
    }
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - 443, frame_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if ((flags & ImGuiSelectableFlags_AllowItemOverlap) || (g.LastItemData.InFlags & ImGuiItemFlags_AllowOverlap)) { button_flags |= ImGuiSelectableFlags_AllowItemOverlap; }
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? ImColor(200, 200, 200, 255) : ImColor(113, 123, 134), g.IO.DeltaTime * 6.f);
