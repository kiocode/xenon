...
void CWaypoints::SetWaypoint(std::string title, const Vec2& pos, ImColor color) {
	m_vWaypoints.push_back(Waypoint(title, pos, color));
}

void CWaypoints::SetWaypoint(std::string title, const Vec3& pos, ImColor color) {
	m_vWaypoints.push_back(Waypoint(title, pos, color));
}
...
void CWaypoints::RenderInRadar(std::function<bool(ImVec2)> fnIsPointInRadar, Vec2 localPos, float defaultScale, float radarSize, float zoomFactor, ImVec2 radarCenter) {
...
void CWaypoints::RenderInRadar(std::function<bool(ImVec2)> fnIsPointInRadar, Vec3 localPos, float defaultScale, float radarSize, float zoomFactor, ImVec2 radarCenter) {
	for (const Waypoint& waypoint : m_vWaypoints) {
