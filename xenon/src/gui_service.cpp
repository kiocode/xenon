...
bool UIService::CreateDeviceUI() {
	...
	pSwapChain->GetBuffer(0U, IID_PPV_ARGS(&back_buffer));

	if (back_buffer) {
		pDevice->CreateRenderTargetView(back_buffer, nullptr, &mainRenderTargetView);
		back_buffer->Release();
		return true;
	}
	return false;
...
void UIService::EndRenderUI()
{
	...
	pContext->OMSetRenderTargets(1U, &mainRenderTargetView, nullptr);
...
HRESULT __stdcall UIService::hkPresent(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags) {
	if (!init)
	{
		if (SUCCEEDED(pSwapChain->GetDevice(__uuidof(ID3D11Device), (void**)&pDevice)))
		{
			...
			pBackBuffer->Release();
			oWndProc = (WNDPROC)SetWindowLongPtr(window, GWLP_WNDPROC, (LONG_PTR)UIService::WndProcWrapper);
			CreateImGuiUI();
			init = true;
